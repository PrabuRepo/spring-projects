...
public class SegmentTree {
	...
	public static void main(String[] args) {
		...
		int[] arr2 = { 5, 7, 5, 2, 10, 12, 11, 17, 14, 1, 44 };
	 ...
	 */
	// Range Minimum Query
	public void rangeMinQuery(int[] arr, Query[] query) {
		...
		for (int i = 0; i < query.length; i++) {
			...
			r = query[i].right;
			System.out.println(
					"Min element in range from " + l + " to " + r + " : " + queryMin(segmentTree, l, r, 0, n - 1, 0));
		}
	}

	private void buildSegmentTree(int[] arr, int[] segmentTree, int low, int high, int pos) {
	...
	private int queryMin(int[] segmentTree, int qLow, int qHigh, int arrLeft, int arrRight, int pos) {
		...
		return Integer.min(queryMin(segmentTree, qLow, qHigh, arrLeft, mid, 2 * pos + 1),
				queryMin(segmentTree, qLow, qHigh, mid + 1, arrRight, 2 * pos + 2));
	}

	// Given an array of integers, evaluate queries of the form LCM(l, r).
	public void rangeLCMQuery(int[] a, Query[] query) {
		int n = a.length, l, r;
		// Initialize segment tree with expected size
		int[] segmentTree = new int[findSegmentTreeSize(n)];

		// Build the segment Tree
		buildSegmentTreeForLCM(a, segmentTree, 0, n - 1, 0);
		...
		for (int i = 0; i < query.length; i++) {
			...
			r = query[i].right;
			System.out.println(
					"LCM element in range from " + l + " to " + r + " : " + queryLCM(segmentTree, l, r, 0, n - 1, 0));
		}

	}

	private void buildSegmentTreeForLCM(int[] arr, int[] segmentTree, int low, int high, int pos) {
		/*If there is one element in array, store it in current node of segment tree and return*/
		if (low == high) {
			segmentTree[pos] = arr[low];
			return;
		...
		int mid = (low + high) / 2;
		buildSegmentTreeForLCM(arr, segmentTree, low, mid, 2 * pos + 1);
		buildSegmentTreeForLCM(arr, segmentTree, mid + 1, high, 2 * pos + 2);
		segmentTree[pos] = Utils.lcm(segmentTree[2 * pos + 1], segmentTree[2 * pos + 2]);
	}

	private int queryLCM(int[] segmentTree, int qLow, int qHigh, int arrLeft, int arrRight, int pos) {
		/*If segment of this node is a part of given range, then return the min of the segment*/
		...
		int mid = (arrLeft + arrRight) / 2;
		// If a part of this segment overlaps with the given range
		return Utils.lcm(queryLCM(segmentTree, qLow, qHigh, arrLeft, mid, 2 * pos + 1),
				queryLCM(segmentTree, qLow, qHigh, mid + 1, arrRight, 2 * pos + 2));
	}

	// Naive Approach
	public void rangeGCDQuery(int[] a, Query[] query) {
		int n = a.length, l, r;
		// Initialize segment tree with expected size
		int[] segmentTree = new int[findSegmentTreeSize(n)];

		// Build the segment Tree
		buildSegmentTreeForGCD(a, segmentTree, 0, n - 1, 0);
		...
		for (int i = 0; i < query.length; i++) {
			...
			r = query[i].right;
			System.out.println(
					"GCD element in range from " + l + " to " + r + " : " + queryGCD(segmentTree, l, r, 0, n - 1, 0));
		}

	}

	private void buildSegmentTreeForGCD(int[] arr, int[] segmentTree, int low, int high, int pos) {
		/*If there is one element in array, store it in current node of segment tree and return*/
		if (low == high) {
			segmentTree[pos] = arr[low];
			return;
		...
		int mid = (low + high) / 2;
		buildSegmentTreeForGCD(arr, segmentTree, low, mid, 2 * pos + 1);
		buildSegmentTreeForGCD(arr, segmentTree, mid + 1, high, 2 * pos + 2);
		segmentTree[pos] = Utils.gcd(segmentTree[2 * pos + 1], segmentTree[2 * pos + 2]);
	}

	private int queryGCD(int[] segmentTree, int qLow, int qHigh, int arrLeft, int arrRight, int pos) {
		/*If segment of this node is a part of given range, then return the min of the segment*/
		if (qLow <= arrLeft && qHigh >= arrRight) // Total Overlap
			return segmentTree[pos];
		...
		int mid = (arrLeft + arrRight) / 2;
		// If a part of this segment overlaps with the given range
		return Utils.gcd(queryGCD(segmentTree, qLow, qHigh, arrLeft, mid, 2 * pos + 1),
				queryGCD(segmentTree, qLow, qHigh, mid + 1, arrRight, 2 * pos + 2));
	}
